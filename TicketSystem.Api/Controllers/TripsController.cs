...
return Ok(new ApiResponse<TripDetailsDto>(trip, "Detalhes da viagem encontrados com sucesso"));
}
...
[HttpPost("with-seats")]
public async Task<IActionResult> CreateWithSeats([FromBody] CreateTripDto createTripDto)
{
var trip = await _tripService.CreateTripWithSeatsAsync(createTripDto);
return CreatedAtAction(nameof(GetDetails), new { id = trip.Id },
new ApiResponse<TripDetailsDto>(trip, "Viagem criada com assentos gerados automaticamente"));
}
